...
2. <strong>计算注意力分数：</strong> 为了确定当前词元（由Q代表）应该对其他所有词元（由K代表）投入多少关注，我们计算当前词元的Q与其他所有词元的K的点积。这个分数衡量了两者之间的相关性。
   ...
   >点积的“符号形式”，含义：直接写“向量点积”，不展开细节。

   $$
   Q_i \cdot K_j = \sum_{d=1}^{D} Q_{i,d} K_{j,d}
   $$

   >向量点积的“展开形式”， 含义：逐维相乘再求和。
   >
   >👉 这里有三个下标：
   >
   >- i：query 的 token
   ...
   **这个等价于矩阵形式：**$QK^{T}$​
   ...
   在 Transformer 里，我们不会一个一个词算，而是一次性矩阵运算。

   📌 **矩阵定义：**

   $Q \in \mathbb{R}^{n \times d} \quad (n个词，每个d维)$

   $K \in \mathbb{R}^{n \times d} \quad (n个词，每个d维)$

   📌 **转置后：**

   $K^{T} \in \mathbb{R}^{d \times n} \quad (转置后矩阵，维度变为 d×n)$
   ...
   **它和你那个公式的关系**
   ...
   👉 第 i 行第 j 列 = 第 i 个 Query 和第 j 个 Key 的点积.
   ...
   QK^T =
   \begin{bmatrix}
   我\cdot我 & 我\cdot喜欢 & 我\cdot吃 & 我\cdot苹果 \\
   喜欢\cdot我 & 喜欢\cdot喜欢 & 喜欢\cdot吃 & 喜欢\cdot苹果 \\
   吃\cdot我 & 吃\cdot喜欢 & 吃\cdot吃 & 吃\cdot苹果 \\
   苹果\cdot我 & 苹果\cdot喜欢 & 苹果\cdot吃 & 苹果\cdot苹果 \\
   \end{bmatrix}
   $$
   👉 这是一个 **“注意力关系矩阵”**
...
3. <strong>缩放（Scaling）：</strong> 将计算出的分数除以一个缩放因子 $\sqrt{d_k}$（ $d_k$​ 是K向量的维度）。这一步是为了在反向传播时获得更稳定的梯度，防止点积结果过大导致Softmax函数进入饱和区。
   ...
   其实 Transformer 做的事情是：

   👉 控制每一维的方差是常数级（$O(1)$）

   这样：
   $$
   \sum_{t=1}^{d_k} Q*_{i,t} K_*{j,t}
   $$
   ...
   >**权重初始化（关键）**
   >
   >线性层一般用 Xavier/Glorot 初始化（或类似）：
   >
   >$$ (W_Q)_{a,b} \sim \mathcal{N}\left(0, \frac{1}{d_{\text{model}}}\right) $$
   >
   >这不是 1，而是 按维度缩放过的方差。
   >
   >---
   ...
   >看一维：
   >
   >$$ Q_{i,t} = \sum_{k=1}^{d_{\text{model}}} X_{i,k} (W_Q)_{k,t} $$
   >
   >如果假设：
   >
   >*   $\text{Var}(X_{i,k}) \approx 1$ （后面解释为什么成立）
   >*   $\text{Var}((W_Q)_{k,t}) = \frac{1}{d_{\text{model}}}$
   >
   >那么：
   >
   >$$ \text{Var}(Q_{i,t}) = \sum_{k=1}^{d_{\text{model}}} \text{Var}(X_{i,k}) \cdot \text{Var}((W_Q)_{k,t}) \approx d_{\text{model}} \cdot 1 \cdot \frac{1}{d_{\text{model}}} = 1 $$
   >
   >> 结果：方差自动被“调到 1”
   >
   >**3 那输入 $X$ 为什么方差 $\approx 1$？**
   >
   >因为 Transformer 里有：
   >- **LayerNorm**（层归一化）
   >
   ...
   >不是：
   >
   >$$ Q_{i,t} \sim \mathcal{N}(0, 1) \quad \text{（写死）} $$
   >
   >而是：
   >
   >$$ Q_{i,t} \approx \mathcal{N}(0, 1) \quad \text{（被系统"维持"在这个量级）} $$
   >
   >---
   >
   >**5 为什么论文直接用 1？**
   ...
   >直接近似成：$$ 1 $$
   >
   >推导更干净：$$ \text{Var}(Q \cdot K) \approx d_k $$
   >
   >
   >
   >📌 一句话总结
   ...
   >\- LayerNorm 控制激活方差
   >
   >\- → 最终让每一维 稳定在方差 $\approx 1$
   >
   >> 所以理论推导可以放心写：$$ Q_{i,t} \sim \mathcal{N}(0, 1) $$
...
4. <strong>Softmax归一化：</strong> 将缩放后的分数通过一个Softmax函数，使其转换为一组总和为1的概率分布。这些概率就是“注意力权重”，表示在当前位置，每个输入词元所占的重要性。
   ...
   >
   >所以：
   >- AttentionWeights 也是 $\mathbb{R}^{n \times n}$
   >
   >👉 含义：
   >每个 token 对所有 token 的"注意力分布"。
...
>
>结果：$$ \text{Output} \in \mathbb{R}^{n \times d_v} $$
>
>---
>
>**4 关键结论**
...
>✅ Output 是一个矩阵
>✅ 每一行对应一个 token 的新表示
>✅ 每个 token 的表示已经融合了全序列信息
>
>---
>
>**5 用一句话理解**
>
>> Attention 输出不是“一个向量”，而是“每个词都更新后的向量序列”。

![b5043340-dcde-4c8c-b4e6-c72431f9d410](LLM & VLM & Agent.assets/b5043340-dcde-4c8c-b4e6-c72431f9d410.png)
...
* <strong>参考答案：</strong>
  ...
  **位置编码（Positional Encoding, PE）**是一个**与词嵌入维度相同的向量**，其目的是向模型注入关于词元在输入序列中绝对或相对位置的信息。它会与词元的词嵌入（Token Embedding）相加，然后一同输入到Transformer的底层。
...
* **参考答案：**
  ...
  ![48947df6-6c70-4899-9ef1-30509688c820](LLM & VLM & Agent.assets/48947df6-6c70-4899-9ef1-30509688c820.png)
...
* <strong>参考答案：</strong>
  ...
  1. **双编码器架构（Dual-Encoder Architecture）：**
     ...
     * <strong>图像编码器（Image Encoder）：</strong> 通常是一个标准的视觉模型，如ResNet或Vision Transformer (ViT)，负责将输入的图像转换成一个高维的特征向量。
       ...
       >
       >```
       >224×224 → 16×16 patch → 14×14 = 196 个 patch
       >```
       >
       >------
       >
       ...
       >
       >```
       >16×16×3 → 展平 → 线性映射 → d维向量
       >```
       >
       >------
       >
       >**Step 3：加一个 CLS token**
       >
       >类似 NLP：
       >
       >```
       ...
       >
       >
       >**为什么 ViT 比 CNN 更适合 CLIP？**
       >
       >ViT 更适合 CLIP，是因为它具备更强的全局建模能力、与文本 Transformer 架构对齐、更适合大规模预训练，并且更容易学习跨模态语义对齐。**ViT 比 CNN 更适合 CLIP，主要有五点原因：**
       >
       >1. **全局建模能力强**
       >    self-attention 使每个 patch 能直接建模全局关系，更适合理解整句文本语义。
       >2. **与文本编码器结构一致**
       >    图像和文本都用 Transformer，更容易学习共享嵌入空间。
       >3. **更适合大规模预训练**
       >    ViT 在海量数据（如 CLIP 的 4 亿图文对）下表现优于 CNN。
       >4. **更偏语义而非纹理**
       >    更容易对齐自然语言中的概念表达。
       >5. **更适合对比学习目标**
       >    输出 embedding 更全局一致，有利于图文对齐。

       ![](LLM & VLM & Agent.assets/ce582b70-cc66-4e78-802f-7d6bdc2db6bd.png)
...
#### <u>4.4 Memory是 Agent 的一个关键模块。请问如何为 Agent 设计短期记忆和长期记忆系统？可以借助哪些外部工具或技术？</u>
...
#### <u>4.8 什么是多智能体系统？让多个 LLM Agent 协同工作相比于单个 Agent 有什么优势？又会引入哪些新的复杂性？</u>
...
#### <strong><u>4.12 你用过哪些Agent框架？选型是如何选的？你最终场景的评价指标是什么？</u></strong>
